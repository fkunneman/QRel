...
# SEEDS_PATH=os.path.join(DATA_PATH, 'seeds_plus.json')
SEEDS_PATH=os.path.join(DATA_PATH, 'seeds_plus_all.json')
...
class GoeieVraag():
    def __init__(self, w2v_dim=300, w2v_window=10, evaluation=False, categorization=False):
        ...
        self.nlp = spacy.load('nl_core_news_sm',disable=['tagger', 'parser', 'ner'])
    ...
    def load_datasets(self):
        ...
        if self.evaluation:
            ...
            self.seeds_text = {}
            for q in self.seeds:
                self.qid_tokens[q['id']] = q['tokens']
                self.seeds_qids.append(q['id'])
                # self.seeds_text[q['id']] = q['text']
                self.seeds_text[q['id']] = q['questiontext']
            self.seeds_qids = set(self.seeds_qids)
    ...
    def init_bm25(self, corpus):
        ...
        # self.in_seeds = {}
        for row in corpus:
            # if set([row['id']]) & self.seeds_qids:
            #     self.in_seeds[row['id']] = True
            # else:
            #     self.in_seeds[row['id']] = False
            ids.append(row['id'])
        ...
        for q1id in traindata:
            # if set([q1id]) & self.seeds_qids:
            #     self.in_seeds[q1id] = True
            # else:
            #     self.in_seeds[q1id] = False
            self.labeled_ids.append(q1id)
            for i,q2id in enumerate(traindata[q1id]):
                self.labeled_ids.append(q2id)
                pair = traindata[q1id][q2id]
                if i == 0:
                    labeled_txt.append(pair['q1'])
                labeled_txt.append(pair['q2'])
        for qid in list(set(self.labeled_ids) - set(ids)):
            ids.append(qid)
            questions.append(labeled_txt[self.labeled_ids.index(qid)])
    ...
    def init_ensemble(self):
        ...
        #         pair = traindata[q1id][q2id]
        #         if not set([q1id]) & set(ids):
        #             ids.append(q1id)
        #             questions.append(pair['q1'])
        #         if not set([q2id]) & set(ids):
        #             ids.append(q2id)
        #             questions.append(pair['q2'])
