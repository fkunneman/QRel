...
class TopicExtractor:
        ...
        for topic in topics:
            ...
            if len(tokens) > 1:
                ...
                if not len(startindices) == 1:
                    print("No single start index for",topic.encode('utf-8'),' '.join([x[0] for x in lemma_sequence]).encode('utf-8'),"startindex",startindices)
    ...
    def extract_entropy(self,question):
        ...
        lemma_ngrams = sum(lemma_ngrams,[])
        entities_entropy = []
        filtered_entities = self.filter_entities(list(set(lemma_ngrams) & self.entropy_set),question)
